...
public partial class EditCountryWindow : Window
{
    ...
    public EditCountryWindow(Country selected)
    {
        InitializeComponent();
        _id = selected.IdCountry;
        txtValue.Text = selected.CountryName;
    }
    ...
    private void btnSave_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var item = context.Countries.Find(_id);
            if (item == null)
            {
                return;
            }

            item.CountryName = txtValue.Text.Trim();
            context.SaveChanges();
            DialogResult = true;
            Close();
        }
